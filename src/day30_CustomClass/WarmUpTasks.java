...
public class WarmUpTasks {
    ...
    public static void main(String[] args) {
        ...
        Collections.swap(numbers, 0, numbers.size() - 1);//swap the first and last elements of an ArrayList
        ...
        int size = list.size(); //the old size before removing zeros

        list.removeAll(Arrays.asList(0)); //[1, 2, 3, 4]

        System.out.println(list);

        int newSize = list.size();//the new size after removing zeros

        int totalNumberOfZeros = size - newSize;

        System.out.println("Total number of Zeros = " + totalNumberOfZeros);//4
